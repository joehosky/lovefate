                ...
                if (!n.data("isActive")) {
                    ...
                    var s = function() {
                        ...
                        B = function() {
                            ...
                            a = '<div class="ic_header">';
                            a += '<div class="ic_prev"></div>';
                            a += '<div class="ic_next"></div>';
                            a += '<div class="ic_month"><select class="ic_month_select">';
                            ...
                            a += "</select></div>";
                            a += '<div class="ic_year"><select class="ic_year_select">';
                            for (b = q; b <= r; b++) a = b === parseInt(d.format("YYYY")) ? a + ('<option value="' + b + '" selected="selected">' + b + "</option>") : a + ('<option value="' + b + '">' + b + "</option>");
                            a += "</select></div>";
                            a += "</div>";
                            if (c.sundayFirst) {
                                a += '<table class="ic_weekset"><tr>';
                                for (b = 0; 7 > b; b++) a += "<td>" + h.day(b).format("dd") + "</td>";
                                a += "</tr></table>";
                                a += '<table class="ic_dayset"><tr>';
                                for (b = 0; b < k; b++) a +=
                                    '<td class="ic_day_empty">&nbsp;</td>';
                                for (b = 1; b <= y; b++) a = moment(d).date(b).format("D.M.YYYY") === t.format("D.M.YYYY") ? a + ('<td class="ic_dayall ic_today">' + b + "</td>") : g && moment(d).date(b).format("D.M.YYYY") === g.format("D.M.YYYY") ? a + ('<td class="ic_dayall ic_day_selected">' + b + "</td>") : a + ('<td class="ic_dayall">' + b + "</td>"), (k + b) / 7 === Math.floor((k + b) / 7) && (a += "</tr><tr>");
                                b = p
                            } else {
                                a += '<table class="ic_weekset"><tr>';
                                ...
                                a += '<table class="ic_dayset"><tr>';
                                k = 0 < k ? k - 1 : 6;
                                for (b = 0; b < k; b++) a += '<td class="ic_day_empty">&nbsp;</td>';
                                for (b = 1; b <= y; b++) a = moment(d).date(b).format("D.M.YYYY") === t.format("D.M.YYYY") ? a + ('<td class="ic_dayall ic_today">' + b + "</td>") : g && moment(d).date(b).format("D.M.YYYY") === g.format("D.M.YYYY") ? a + ('<td class="ic_dayall ic_day_selected">' + b + "</td>") : a + ('<td class="ic_dayall">' + b + "</td>"), (k + b) / 7 === Math.floor((k + b) / 7) && (a += "</tr><tr>");
                                1 > p && (p = 7);
                                b = p - 1
                            }
                            for (; 6 > b; b++) a += '<td class="ic_day_empty">&nbsp;</td>';
                            a += "</tr></table>";
                            a += "</div>";
                            C()
                        },
                        C = function() {
                            ...
                            e = n.find(".ic_prev");
                            l = n.find(".ic_next");
                            m = n.find(".ic_month_select");
                            v = n.find(".ic_year_select");
                            x = n.find(".ic_dayall");
...
(function(f) {
    var t = 0,
        u, m = f(document.body),
        e = function() {
            f(".ic_datepicker").css("left", "-9999px").css("top", "-9999px")
        },
        c = {
            init: function(a) {
                var b = f.extend({
                ...
                return this.each(function() {
                    ...
                    if (!a.data("isActive")) {
                        ...
                        var p = function() {
                            ...
                            c.css("left", v + "px").css("top", x + 35 + "px");
                            (h = a.prop("value")) && (h !== d && 0 > b.format.indexOf("L")) && (k = moment(h, b.format), k.isValid() && c.ionCalendar("update", {
                                startDate: h
                            }))
                        };
                        (function() {
                            u = '<div class="ic_datepicker" id="ic_datepicker-' + z.pluginCount + '"></div>';
                            m.append(u);
                            c = f("#ic_datepicker-" +
                                z.pluginCount);
                            c.ionCalendar(b);
